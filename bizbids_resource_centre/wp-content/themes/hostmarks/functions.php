...
function getMainMenu($menulocation){
  ...
  $menuItems = wp_get_nav_menu_items( $locations[ $menulocation ] );
    if(empty($menuItems))
     ...
     <?php echo do_shortcode('[do_widget id=dc_jqmegamenu_widget-3]');?>
...
$return = '<div class="fb-share-button" data-href="' . $url . '" data-width="' . $width . '" data-type="' . $layout . '"></div><br><br>';
return $return;
}
...
function catch_that_image() {  
    ...
    if(emptyempty($first_img)){  
            ...
            $first_img = "wp-content/themes/hostmarks/images/logo.jpg";  
        }  
    return $first_img;  
}  
?>
